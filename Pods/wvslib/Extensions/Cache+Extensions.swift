...
public extension Cache where T == CodableImage {
    ...
    func fetchImage(_ base: String, _ path: String, _ size: CGSize? = nil, _ tint: UIColor? = nil, _ completion: @escaping (_ image: UIImage?) -> ()) {
        self.item(forKey: path) { (codable) in
            guard let image = codable?.image else {
                ImageRequest(base: base, path: path).send({ (result) in
                    switch result {
                    case .success(let image):
                        completion(self.updateImage(image.0, size, tint))
            ...
            completion(self.updateImage(image, size, tint))
        }
    }
